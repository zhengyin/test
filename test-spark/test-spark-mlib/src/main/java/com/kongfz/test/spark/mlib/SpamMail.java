 ...
 * @date Created on 2018/11/9 5:30 PM
 */
public class SpamMail {
    public static void main(String[] args) {
        SparkConf sparkConf = new SparkConf().setAppName("JavaBookExample").setMaster("lock[2]");
        JavaSparkContext sc = new JavaSparkContext(sparkConf);
        // Load 2 types of emails from text files: spam and ham (non-spam).
        // Each line has text from one email.
        JavaRDD<String> spam = sc.textFile("test-spark/test-spark-mlib/files/spam.txt");
        JavaRDD<String> ham = sc.textFile("test-spark/test-spark-mlib/files/ham.txt");
